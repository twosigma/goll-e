...
var EditorModel = createClass({
  ...
  instance: {
    ...
    _attachManualLayoutListener: function() {
      ...
      var self = this;
      this._layoutChangedEventHandle = this.get('layout').after(['add', 'remove', 'update', 'change'], function() {
        // Generate GLL from the layout model and set layoutText.
        self.set('layoutText', generateGll(self.get('layout')));
      });
    },
  ...
  attrs: {
    ...
    layout: {
      ...
      setter: function(val) {
        this.get('layout').removeBubbleTarget(this);
        val.addBubbleTarget(this);
