...
var convertCoordinates = function(p, a, newOrigin, scale) {
  scale = scale || 1;
  var p1 = {
  ...
  if (newOrigin) {
    var offset1 = convertCoordinates(newOrigin, a, null, scale);
    p.x -= offset1.x;
    p.y -= offset1.y;
  }
...
var unConvertCoordinates = function(p1, a, originInDest, scale) {
  var originInSource = convertCoordinates(originInDest, a, null, scale);
  var p = {
    x: p1.x + originInSource.x,
    y: p1.y + originInSource.y
  };

  return convertCoordinates(p, -a, null, 1/scale);
};
...
var ReroutePoint = createClass({
  instance: {
    ...
    getAbsolute: function(sourcePos, targetPos) {
      ...
      var p = unConvertCoordinates(p1, angle, offset, scaleFactor);
  ...
  statics: {
    ...
    _getRelativeFromAbsolute: function(x, y, sourcePos, targetPos) {
      ...
      var p1 = convertCoordinates(p, angle, offset, scaleFactor);
