...
/**
 * Unprefix prefixed event attributes
 * @method modernizeEvent
 * @param  {MouseEvent} e
 */
var modernizeEvent = function(e) {
  if (Lang.isValue(e.movementX) || Lang.isValue(e.movementY)) {
    return;
  }

  if (Lang.isValue(e.mozMovementX)) {
    e.movementX = e.mozMovementX;
    e.movementY = e.mozMovementY;
  } else if (Lang.isValue(e.webkitMovementX)) {
 ...
 */
var augmentEvent = function(e, scaleFactor) {
  e.scaledMovementX = e.movementX / scaleFactor;
  e.scaledMovementY = e.movementY / scaleFactor;
};
 ...
 * @param {Number} scaleFactor - the scale of the view to correct for. For example 2 for 2x zoom.
 *   Properties scaledMovementX and scaledMovementY which compensate for the scaleFactor
 *   will be added to the event object.
 * @param {dragCallback} event - the original mouseDown event that starts the pseudo
      drag
 * @returns {void}
 */
var onMouseMove = function(name, pseudoDragCallback, scaleFactor, event) {
  if (this.state[name + '_dragging'] && pseudoDragCallback) {
    modernizeEvent(event);
    augmentEvent(event, scaleFactor);
 ...
 * @param {event} event - the original mouseDown event that starts the pseudo
      drag
 * @param {Number} [scaleFactor=1] - the scale of the view to correct for. For example 2 for 2x zoom.
 *   Properties scaledMovementX and scaledMovementY which compensate for the scaleFactor
 *   will be added to the event object.
 * @returns {void}
 */
var mouseDownDrag = function(name, pseudoDragStartCallback, pseudoDragEndCallback, pseudoDragCallback, scaleFactor, event) {

  if (!Lang.isNumber(scaleFactor)) {
    scaleFactor = 1;
  }
  ...
  window.onmousemove = onMouseMove.bind(this, name, pseudoDragCallback, scaleFactor);
