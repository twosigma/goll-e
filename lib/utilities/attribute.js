...
Attribute.prototype = {
  ...
  /**
   * Throw an error if an attribute with name does not exist.
   * @method  _validateHasAttr
   * @param   {String} name
   * @private
   */
  ...
  setAttrs: function(attrs) {
    ...
    ObjectUtils.each(attrs, function(value, name) {
      var newVal = this._prepareValue(name, value);
      if (newVal !== FAIL) {
        newVals[name] = value;
    ...
    this.fire('change', function() {
      ObjectUtils.each(newVals, function(newVal, name) {
        ...
        this.fire(name + 'Change', function() {
          this._setValueHelper(name, newVal);
  ...
  _setAttr: function(name, value, opts, force, onInit) {
    value = this._prepareValue(name, value, opts, force, onInit);
    if (value === FAIL) {
      return;
    }
    ...
    */
    this.fire('change', function() {
      this.fire(name + 'Change', function() {
        this._setValueHelper(name, value, onInit);
      }.bind(this), adtlEventParams);
    }.bind(this), adtlEventParams);
  ...
  _prepareValue: function(name, value, opts, force, onInit) {
   ...
   * @method  _setValueHelper
   * @private
   */
  _setValueHelper: function(name, value, onInit) {
    var state = this._attrStates[name];
