...
var springForce = function(edge) {
  ...
  var addRepulsion = function(layout) {
    var oldPos = layout.get('position');
    var newPos = {
      x: oldPos.x + repulsionX,
      y: oldPos.y + repulsionY
    };
    layout.set('position', newPos);
  };

  var subtractRepulsion = function(layout) {
    var oldPos = layout.get('position');
    var newPos = {
      x: oldPos.x - repulsionX,
      y: oldPos.y - repulsionY
  ...
  if (!vertexToPinned) {
    addRepulsion(vertexToLayout);
  }
  if (!vertexFromPinned) {
    subtractRepulsion(vertexFromLayout);
  }
...
var portLayoutHelper = function(ports, cardinalDirection, maxPortsPerSide) {
  for(var j = 0; j < ports.length; j++) {
    var posPercentage = 12 + 15 * (j % maxPortsPerSide);
    if(j < maxPortsPerSide) {
      if(!ports[j].get('isPinned')) {
        ports[j].set('position', new CardinalPortPosition({direction: cardinalDirection, percentage: posPercentage}));
      }
    } else{
...
var layoutPorts = function(graph) {
  var verts = graph.get('vertices');
  var maxPortsPerSide = 6;
  for(var i = 0; i < verts.length; i++) {
    var ins = verts[i].get('inputs');
    var outs = verts[i].get('outputs');
    portLayoutHelper(ins, CardinalDirection.WEST, maxPortsPerSide);
    portLayoutHelper(outs, CardinalDirection.EAST, maxPortsPerSide);
  }
};
