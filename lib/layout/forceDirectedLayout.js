...
var getEdgeDistance = function(edge){
  ...
  var toLayout = to.getLayout();
  var fromLayout = from.getLayout();

  var toPos = toLayout.get('position');
  var fromPos = fromLayout.get('position');
...
var springForce = function(edge){
  ...
  if(vertexToLayout.get('isPinned') === true && vertexFromLayout.get('isPinned') === true){
    return;
  }
  ...
  if(vertexToLayout.get('isPinned') === false && vertexFromLayout.get('isPinned') === false){
    addRepulsion(vectors.get(vertexTo));

    addRepulsion(vectors.get(vertexFrom), -1);

  }else if(vertexToLayout.get('isPinned') === false){
    addRepulsion(vectors.get(vertexTo), 2);

  }else if(vertexFromLayout.get('isPinned') === false){
...
var layOutGraph = function(graph){
  ...
  var changePositionInDirection = function(vector, vertex, direction) {
    var vertexLayout = vertex.getLayout();
    var position = vertexLayout.get('position');
    ...
    vertexLayout.set('position', position);
  };
