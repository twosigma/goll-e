  ...
  for(var i = 0; i < verts.length; i++) {
    ...
    for(var j = 0; j < ins.length; j++) {
      ...
      if(j < maxPortsPerSide) {
        if(!ins[j].get('isPinned')) { 
          ins[j].set('position', new CardinalPortPosition({direction: CardinalDirection.EAST, percentage: posPercentage}));
        }
      } else{
        if(!ins[j].get('isPinned')) {
          ins[j].set('position', new CardinalPortPosition({direction: CardinalDirection.NORTH, percentage: posPercentage}));
        }
      }
    }
    for(j = 0; j < outs.length; j++) {
      posPercentage = 12 + 15 * (j % maxPortsPerSide);
      if(j < maxPortsPerSide) {
        if(!outs[j].get('isPinned')) {
          outs[j].set('position', new CardinalPortPosition({direction: CardinalDirection.WEST, percentage: posPercentage}));
        }
      } else{
        if(!ins[j].get('isPinned')) {
          outs[j].set('position', new CardinalPortPosition({direction: CardinalDirection.SOUTH, percentage: posPercentage}));
        }
      }
    }
  }
