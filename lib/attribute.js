...
Attribute.prototype = {
   ...
   * - value: the initial or "default" value
   * - valueFn: a function to return an initial value or a string with the name
   ...
   * - getter: a custom get handler function (or name of an instance method)
   ...
   * - setter: a custom set handler function (or name of an instance method)
   *   It receives the value that would have been set and has a chance to massage it.
   *   Receives: current value, name of attribute.
   *   returns: the value that will actually be set
   * - validator: a predicate function (or name of an instance method) 
   ...
   * - readOnly: boolean. If true, it cannot be set.
   * - writeOnce: if true, it can be set once only.
   * - initOnly: if true it can only be set in the constructor using `classCreate` utility
   *
  ...
  _setAttr: function(name, value, opts, force, onInit) {
    ...
    if (!onInit && config[OPTS.INIT_ONLY]) {
